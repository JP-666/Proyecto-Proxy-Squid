...
apt update >> log
DEBIAN_FRONTEND=noninteractive apt install -y -qq apache2-utils iptables squid-openssl iptables-persistent isc-dhcp-server nginx php-fpm openssh-server git freeradius freeradius-mysql mariadb-common mariadb-server php-mysql mysql-common mariadb-client mariadb-server sudo jq >> log
echo "[2] IPTABLES"
