...
then
	...
	cd /bin
	versionphp=$(ls php7* || ls php8* || ls php9* || ls php6* || ls php5* || ls php4* || ls php3* || ls php2* || echo "No se ha encontrado PHP!") # Buscamos nuestro PHP
	cp -rvf /etc/squid/ssl_cert/squid_ca.pem /srv/certi.pem # Copiamos el certificado para descarga
	ln -sf /srv/certi.pem /srv/alumnos/certi.pem # Links para el alumno, reduntante si se les permite entrar en la int. de conf.
	echo
	echo "Se va a reiniciar squid, esto tarda un rato..."
	echo
	systemctl restart isc-dhcp-server nginx $versionphp-fpm squid
else
	echo "No estamos instalando la interfaz, saltando configs de nginx"
	echo
	echo "Asegurate de darle a los usuarios el certificado de otra manera!"
	echo
	echo "Se va a reiniciar squid, esto tarda un buen rato..."
	systemctl restart isc-dhcp-server squid
fi
